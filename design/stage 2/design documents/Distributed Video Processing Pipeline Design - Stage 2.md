...
Redis does not disappear; it keeps the jobs it is actually best at — dedup keys,
rate limiting, live stream ownership — none of which are in the frame path.
...
| **Redis** | Replicated, but **deliberately non-critical** — it holds only dedup and rate-limit state. Losing it degrades to duplicate detections, not lost work. |
...
| Job / stream registry | **Postgres** | Transactional, queryable, durable. |
| Detections | Downstream, plus `results.v1` | Kafka is the buffer; the system of record is the next service's. |
| Dedup, rate limits | **Redis** | TTL-scoped, high-churn, loss-tolerant. |
...
|---|---|---|
| **VideoAnalyzer · API** | Stage 1 `VideoAnalyzer` (the FastAPI half) | I/O-bound; scales on request rate |
| **VideoAnalyzer · Decoder workers** | Stage 1 `VideoAnalyzer` (`FramePipeline`) | CPU-bound; scales on queue depth |
| **StreamIngestor** | *new* | Live cameras have no job lifecycle — a subscription, not a job |
...
flowchart LR
    ...
    ING["2. StreamIngestor<br/>decode + JPEG, owns N cameras"]
    ...
    KR[["8. results.v1"]]
    SINK["9. ResultSink"]
    CTL["VideoAnalyzer · API<br/>POST /streams, camera registry"]
...
What gets replaced: the synchronous 200, the claim-check, Redis in the frame
path, the 120 s staleness heuristic, and RabbitMQ.
